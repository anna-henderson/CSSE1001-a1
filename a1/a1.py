...
def create_empty_board(board_size: int) -> list[str]:
    ...
    Pre-condition:
        board_size must be between 2 and 9
    """
    return [EMPTY_SQUARE * board_size] * board_size


def get_square(board: list[str], position: Position) -> str:
    """Returns the character present at the given (row, column) position within
    the given board.
...
def change_square(
    board: list[str], position: Position, new_square: str
) -> None:
    """Replaces the character at the given (row, column) position with new
    string.
...
def coordinate_to_position(coordinate: str) -> Position:
    """Returns the (row, column) position tuple corresponding to the given
    coordinate.

    Parameters:
        coordinate: The coordinate in the format of column, row

    Returns:
        Position as a tuple of row, column coordinates

    Pre-condition:
        Coordinate must consist of two characters where the first is a capital
        letter from "A" to "I" and the second is a single digit character
...
def can_place_ship(board: list[str], ship: list[Position]) -> bool:
    ...
    Returns:
        Returns True if the ship can be placed on the board, else False
...
def place_ship(board: list[str], ship: list[Position]) -> None:
    """Places the ship on the board by changing all positions from the ship to
    "O" (AACTIVE_SHIP_SQUARE).
    ...
    Pre-condition:
        ship should be able to be placed on the board according to
        can_place_ship.
    """
    for position in ship:
...
def attack(board: list[str], position: Position) -> None:
    """Attempts to attack the cell at the (row, column) position within the
    board and places result of attack on the board.
...
def display_board(board: list[str], show_ships: bool) -> None:
    """Prints the board in a human-readable format. If show_ships is False, the
    board will print active ship squares as empty squares.
...
def display_game(
    ...
    """Prints the overall game state. The game state consists of player 1's
        health and board state,
...
def is_valid_coordinate(coordinate: str, board_size: int) -> Result:
    ...
    Returns:
        Returns tuple of False and a string describing the issue if invalid
        coordinate, else returns tuple of True and empty string
    """
    if len(coordinate) != 2:
        return (False, INVALID_COORDINATE_LENGTH)

    # Check if the first digit does not correlate to a valid letter
    if LETTER_TO_INDEX.get(coordinate[0]) is None:
        return (False, INVALID_COORDINATE_LETTER)

    # Check if the first digit is a valid letter but is out of range
    if board_size <= LETTER_TO_INDEX.get(coordinate[0]):
        return (False, INVALID_COORDINATE_LETTER)

    # Check if the second digit is not a valid integer or is out of range
    if not coordinate[1].isdigit() or int(coordinate[1]) not in range(
        1, board_size + 1
    ):
...
def is_valid_coordinate_sequence(
    ...
    Parameters:
        coordinate_sequence: The coordinates as a string separated by a comma
        ship_length: The length of the ship
        board_size: The size of the board

    Returns:
        A tuple containing a boolean (which is True if there are exactly ship
        length coordinates which are all valid, and False otherwise), and a
        string containing a message to describe the issue (if any) with the
        coordinate sequence.
    """
    ...
    for coord in coords:
        ...
        if is_valid_coordinate(coord, board_size)[0] is False:
            return is_valid_coordinate(coord, board_size)
    return SUCCESS


def build_ship(coordinate_sequence: str) -> list[Position]:
    """Returns the list of (row, column) positions corresponding to the
    coordinate sequence.
...
def setup_board(board_size: int, ship_sizes: list[int]) -> list[str]:
    """Allows the user to setup a new board by getting user inputs to place the
    ships.
    ...
    for ship_length in ship_sizes:
        ...
        while ship_placed is False:
            ...
            coords_sequence = input(prompt_for_ship_coordinates(ship_length))

            # If valid coordinate sequence construct a ship
            if is_valid_coordinate_sequence(
                coords_sequence, ship_length, board_size)[
                0
            ]:
                ship = []
                ship_coords_list = coords_sequence.split(",")
...
def play_game() -> None:
    ...
    while get_winner(p1_board, p2_board) is None:
        turn += 1
